...
function initializePopup() {
  ...
  // Load theme from storage first, then apply it
...
async function resetStats() {
  ...
  try {
    ...
    chrome.storage.sync.set({
      ...
      // Nếu là người dùng đăng nhập, gọi API để reset thống kê server
      if (document.body.classList.contains('logged-in')) {
        try {
          ...
          if (!response.ok) {
            console.error('Failed to reset server stats:', response.status);
          }
        } catch (error) {
          console.error('Error resetting server stats:', error);
        }
      }
